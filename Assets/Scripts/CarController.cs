...
public class CarController : MonoBehaviour
{
    ...
    private float radius = 6, brakPower = 0, DownForceValue = 100f, wheelsRPM, driftFactor, lastValue, horizontal, vertical, totalPower;
    ...
    public void ActivateNitrus()
    {
        if (IM.boosting )
        {
            CarEffects.StartNitrous(); // Activate nitrous effects
            rigidbody.AddForce(transform.forward * 10000);
            Debug.Log("Nitrous activated in CarController!");
        }
        else
        {
            CarEffects.StopNitrous(); // Deactivate nitrous effects
            Debug.Log("Nitrous deactivated in CarController!");
        }
    }
}
